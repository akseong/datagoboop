...
sonify_scatter <- function(data, x_var, y_var,  # need to be specified
  ...
  x <- data %>% select({{x_var}}) %>% as_vector()
  y <- data %>% select({{y_var}}) %>% as_vector()
  ...
  if (is.null(order_var)) {
    ...
    ord <- data %>% select({{order_var}}) %>% as_vector()
  }
  ...
  if (is.null(factor_var)) {
    ...
    fac <- data %>% select({{factor_var}}) %>% as_vector()
  ...
  if (is.null(duration)) {
    ...
    if (length(duration) == 1 & is.numeric(duration)) {
      ...
      dur <- data %>% select({{ duration }}) %>% as_vector()  
      ...
      if (is.null(speed)){
        speed <- 1
        if (verbose) message("playback speed set to 1 to match duration scaling. ",
  ...
  if (is.null(speed)){
    speed <- 4
    if (verbose) message("playback speed set to 4",
                         "Consider specifying speed.")
  }
  ...
  if (is.null(volume)) {
    ...
    if (length(volume) == 1 & is.numeric(volume)) {
      ...
      vol <- data %>% select({{ volume }}) %>% as_vector()  
  ...
  if (sum(is.na(x)) > 0 | sum(is.na(y)) > 0) {
    message(sum(is.na(x)), " NAs in x_var and ", 
